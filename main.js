...
class Game {
    ...
    startStory() {
        ...
        this.storyMessages = [
            ...
            "バーテンダー<br>「地下迷宮の話は知っているかい？<br>どうやらあそこには『アビスロード』という強大な魔物が潜んでいて、最近起きている災いの元になっているらしい。」",
            "バーテンダー<br>「奴を倒せば一定期間平和が訪れるが、時が経てばまた復活する厄介な存在だそうだ。」",
            "誰からともなく、名乗りをあげる声があった。",
            `${this.party[0].name}<br>「...なるほどな。この酒場には腕利きが集まってるんだろ？」`,
            "そう言うと、周囲にいた者たちが次々に立ち上がった。",
            `${this.party[1].name}<br>「面白そうじゃないか。ふふ、私も一口乗るよ。」`,
            `${this.party[2].name}<br>「迷宮か...強敵との戦いは悪くない。私も行こう。」`,
            `${this.party[3].name}<br>「よし、じゃあこの4人で決まりだな！早速迷宮へ乗り込もうぜ！」`,
            "こうして、彼らはその場の勢いで恐るべき迷宮へと足を踏み入れることになった――"
        ];
    ...
    handleEnding() {
        ...
        UI.showBlackout(this, "アビスロードの波動が消失した...", 4000, () => {
    ...
    toggleCamp() {
        if (this.state === 'EXPLORE') { this.state = 'CAMP'; document.getElementById('explore-menu').style.display = 'none'; document.getElementById('camp-menu').style.display = 'flex'; this.updateUI(); }
        else if (this.state === 'CAMP') { this.state = 'EXPLORE'; document.getElementById('camp-menu').style.display = 'none'; document.getElementById('explore-menu').style.display = 'flex'; this.discardingItemIdx = -1; this.updateUI(); this.saveGame(); }
    }
